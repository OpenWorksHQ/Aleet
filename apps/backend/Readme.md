...
Deployed by .github/workflows/deploy-backend.yml onto a self-hosted GitHub Actions
runner on EC2, restarted under PM2 as "aleet-backend". Full setup notes are in the
root README.md → "Deployment & Operations". The essentials:

Quality gate: the deploy runs `npm ci`, `node --check` over every file in src/, and
`npm test --workspace=swift-haven-backend` (`node --test src/`) BEFORE PM2 is
restarted, then prunes to production dependencies. A failing test or a syntax error
stops the deploy and leaves the previous build running. Keep `npm test` green — it is
now on the critical path to production.

UPLOAD_DIR — must point outside the runner workspace (default /var/lib/aleet/uploads).
`actions/checkout` runs `git clean -ffdx` on the existing workspace, and `-x` deletes
gitignored files; `uploads` is in .gitignore, so anything stored under the checkout
(driver licenses, vehicle photos, investor documents) is destroyed on the next deploy.
The workflow creates the directory plus its investor/ subdirectory and symlinks
apps/backend/uploads at it so the /uploads static route still resolves. Setting
AWS_S3_BUCKET moves uploads to S3 and makes UPLOAD_DIR moot.

RUN_CRON_JOBS — controls the four in-process setInterval sweeps started in
src/server.js (dispatch escalation 1 min, presence sync 5 min, membership auto-renewal
1 hour, stale booking cancel 15 min). They run inside the API process, so one process
means one copy of every sweep. Must be true on EXACTLY ONE instance. Two instances with it
enabled means membership cards get charged twice. All false means renewals, dispatch
escalation and booking cleanup silently stop.

ALLOW_VERCEL_ORIGINS — opt-in, defaults to false. When true, CORS accepts every
*.vercel.app origin. Put specific preview URLs in ALLOWED_ORIGINS instead.

NODE_ENV=production — set by the deploy. Without it src/seeders/seedGuard.js does not
recognise production and will not refuse to seed or clear the live database, and
src/middleware/errorHandler.js returns raw internal error messages to clients.

apps/backend/.env is generated by the workflow with umask 077 / chmod 600 on a runner
whose workspace persists between jobs. Do not loosen the permissions and do not print
it from a workflow step.